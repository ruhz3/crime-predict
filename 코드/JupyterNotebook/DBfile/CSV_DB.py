...
curs = conn.cursor()
idx = int(input("1.Crime 2.Artificial 3.Weather 4.CCTV 5.Police_Station 6.Resident 7.Floating 8.Adult 9.Apartment 10.Commercial 11.Detached 12.School 13.Traffic 14.Gridnum"))
...
#Adult table
    ...
    sql = "insert into Adult (latitude, longitude, grid) values (%s, %s, %s)"
    f = open('Adult_grid.csv', 'r', encoding='utf-8')
    rd = csv.reader(f)
    for line in rd:
        curs.execute(sql, (line[0], line[1], line[2])) # latitude, longitude, grid
    f.close()

#Apartment table
elif idx == 9:
    sql = "insert into Apartment (latitude, longitude, size, grid) values (%s, %s, %s, %s)"
    f = open('Apartment_grid.csv', 'r', encoding='utf-8')
    rd = csv.reader(f)
    for line in rd:
        curs.execute(sql, (line[0], line[1], line[2], line[3])) # latitude, longitude, size, grid
    f.close()

#Commercial table
elif idx == 10:
    sql = "insert into Commercial (latitude, longitude, size, grid) values (%s, %s, %s, %s)"
    f = open('Commercial_grid.csv', 'r', encoding='utf-8')
    rd = csv.reader(f)
    for line in rd:
        curs.execute(sql, (line[0], line[1], line[2], line[3])) # latitude, longitude, size, grid
    f.close()
...
#Detached table
    ...
    sql = "insert into Detached (latitude, longitude, size, grid) values (%s, %s, %s, %s)"
    f = open('Detached_grid.csv', 'r', encoding='utf-8')
    rd = csv.reader(f)
    for line in rd:
        curs.execute(sql, (line[0], line[1], line[2], line[3])) # latitude, longitude, size, grid
    f.close()
...
#School table
    ...
    sql = "insert into School (latitude, longitude, grid) values (%s, %s, %s)"
    f = open('School_grid.csv', 'r', encoding='utf-8')
    rd = csv.reader(f)
    for line in rd:
        curs.execute(sql, (line[0], line[1], line[2])) # latitude, longitude, grid
    f.close()
...
#Traffic table
    ...
    sql = "insert into Traffic (latitude, longitude, grid) values (%s, %s, %s)"
    f = open('Trafficaccident_grid.csv', 'r', encoding='utf-8')
    rd = csv.reader(f)
    for line in rd:
        curs.execute(sql, (line[0], line[1], line[2])) # latitude, longitude, grid
    f.close()
